...
The mean of total number of steps per day is `r round(mean_steps_per_days,0)`.

The median of total number of steps per day is `r round(median_steps_per_days,0)`.


## What is the average daily activity pattern?

```{r daily_activity_pattern, fig.height=5,fig.width=10}
...
The interval containing the max. average number of steps is `r max_time_interval`, and its value is `r max_time`. This interval means the 5 minutes between 8:35 am and 8:40 am
...
```
1. The number of rows with missing values is `r rows_missing`
...
```{r New_histogram_replace_missing_values,echo=TRUE}
...
The mean of total number of steps per day is `r round(new_mean_steps_per_days,0)`.

The median of total number of steps per day is `r round(new_median_steps_per_days,0)`.
...
```{r comparing_weekday_weekend_time_series, fig.height=8,fig.width=10}
